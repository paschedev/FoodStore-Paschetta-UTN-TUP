...
const cargarCategorias = () => {
  ...
  lista.innerHTML = "";
  const categorias = getCategories().filter(c => !c.eliminado);
  ...
  categorias.forEach(cat => {
    ...
    li.addEventListener("click", (e) => {
      e.preventDefault();
      const filtrados = productos.filter(p => (p as any).categoria && (p as any).categoria.id === cat.id);
  ...
  adminLink?.addEventListener('click', (e) => {
    ...
    if (!user || user.rol.toLowerCase() !== 'admin') {
      ...
      Swal.fire({
        ...
        text: 'Tu cuenta no tiene los privilegios necesarios para entrar aquí.',
